...
# метод Ньютона
def solve(n):
    H = np.zeros((n, 1), dtype=np.float128) # вектор-функция H
    dH = np.zeros((n, n), dtype=np.float128) # вектор-функция производных значений H
    ...
    y = np.zeros((n, 1), dtype=np.float128) # результат решения методом Ньютона, игрики
    y[0] = 0
    y[v.size - 1] = n
